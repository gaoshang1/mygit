...
public class AddFeedbackActivity extends BaseActivity
{
    ...
    private void getData()
    {
        if (service != null)
        {
            try
            {
                String paramet = "http://172.29.53.36/mobile2/supervision/gotoAddUndertakeBack?id=" + guId + "&isSecond=" + isSecond;
                    ...
                    public void onResponse(final String response) throws RemoteException
                    {
                        runOnUiThread(new Runnable()
                        {
                            ...
                            public void run()
                            {
                                ...
                                {
                                    text1.setText(DateUtils.timeToString3(mData.getSup().getNEXT_BACK_DATE().getTime()));
                                }
                            }
                        });
                    }
    ...
    @OnClick({R.id.left_img, R.id.icon_fj, R.id.titles, R.id.text2})
    public void onViewClicked(View view)
    {
        switch (view.getId())
        {
            ...
            case R.id.titles:
                ...
                txt5 = text6.getText().toString();
                txt6 = text7.getText().toString();
                txt7 = text8.getText().toString();
                txt8 = text9.getText().toString();
    ...
    private void saveBack()
    {
        if (service != null)
        {
            try
            {
                ...
                if (isSecond)
                {
                    paramet = "http://172.29.53.36/mobile2/supervision/saveSecondUndertakeBack?userGuid=" + userId + "&undertakeBackGuid=" + mData.getSup().getUNDERTAKE_BACK_GUID() + "&detPlanGuid=" + guId + "&askDate=2018-7-28&endDate=" +
                            txt2 + "&complete=" + txt3 + "&message=" + txt4 + "&progress=" + TohexUtils.strToHex(txt5) + "&problem=" + TohexUtils.strToHex(txt6) + "&proDept=" + TohexUtils.strToHex(txt7) + "&plan=" + TohexUtils.strToHex(txt8);
                } else
                {
                    paramet = "http://172.29.53.36/mobile2/supervision/saveUndertakeBack?userGuid=" + userId + "&undertakeBackGuid=" + mData.getSup().getUNDERTAKE_BACK_GUID() + "&specificGuid=" + guId + "&askDate=2018-7-28&endDate=" +
                            txt2 + "&complete=" + txt3 + "&message=" + txt4 + "&progress=" + TohexUtils.strToHex(txt5) + "&problem=" + TohexUtils.strToHex(txt6) + "&proDept=" + TohexUtils.strToHex(txt7) + "&plan=" + TohexUtils.strToHex(txt8);
        ...
        new Thread(new Runnable()
        {
            ...
            public void run()
            {
                if (service != null)
                {
                    try
                    {
                        String requestUrl = "http://172.29.53.36/servlet/SupFujianUpload?userGUID=" + userId + "&instanceGUID=" + mData.getSup().getUNDERTAKE_BACK_GUID();
                        service.registerCallbacks(new ITaskCallback.Stub()
                        {
                            @Override
                            public void onProgress(float progress, long total) throws RemoteException
