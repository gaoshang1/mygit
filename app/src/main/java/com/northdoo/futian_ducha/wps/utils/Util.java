...
public class Util {
	...
	public static Intent getOpenIntent(Context context, String path,
		...
		// 获得上次打开的文件信息
		String closeFilePath = settingPreference.getSettingParam(
				Define.CLOSE_FILE, "null");
		String packageName = settingPreference.getSettingParam(
				Define.THIRD_PACKAGE, context.getPackageName());
		float ViewProgress = settingPreference.getSettingParam(
				Define.VIEW_PROGRESS, (float) 0.0);
		float ViewScale = settingPreference.getSettingParam(Define.VIEW_SCALE,
				(float) 1.0);
		int ViewScrollX = settingPreference.getSettingParam(
				Define.VIEW_SCROLL_X, 0);
		int ViewScrollY = settingPreference.getSettingParam(
				Define.VIEW_SCROLL_Y, 0);
		String userName = settingPreference.getSettingParam(Define.USER_NAME,
				"newUser");

		// 获取用户设置的参数信息
		String OpenMode = settingPreference.getSettingParam(Define.OPEN_MODE,
				null);
		boolean SendSaveBroad = settingPreference.getSettingParam(
				Define.SEND_SAVE_BROAD, false);
		boolean SendCloseBroad = settingPreference.getSettingParam(
				Define.SEND_CLOSE_BROAD, false);
		boolean IsIsClearBuffer = settingPreference.getSettingParam(
				Define.IS_CLEAR_BUFFER, false);
		boolean IsClearTrace = settingPreference.getSettingParam(
				Define.IS_CLEAR_TRACE, false);
		boolean IsClearFile = settingPreference.getSettingParam(
				Define.IS_CLEAR_FILE, false);
		boolean IsViewScale = settingPreference.getSettingParam(
				Define.IS_VIEW_SCALE, false);
		boolean AutoJump = settingPreference.getSettingParam(Define.AUTO_JUMP,
				false);
		boolean EnterReviseMode = settingPreference.getSettingParam(
				Define.ENTER_REVISE_MODE, false);
		boolean CacheFileInvisible = settingPreference.getSettingParam(
				Define.CACHE_FILE_INVISIBLE, false);
		...
		try {
			...
			if (checkPackage(context, Define.PACKAGENAME_ENT)) {
				...
				intent.setClassName(Define.PACKAGENAME_KING_ENT,
						Define.CLASSNAME);
			} else if (checkPackage(context, Define.PACKAGENAME_KING_PRO)) {
				intent.setClassName(Define.PACKAGENAME_KING_PRO,
						Define.CLASSNAME);
			} else if (checkPackage(context, Define.PACKAGENAME_KING_PRO_HW)) {
				intent.setClassName(Define.PACKAGENAME_KING_PRO_HW,
						Define.CLASSNAME);
			} else {
	...
	public static Intent getPDFOpenIntent(Context context, String path,
			boolean isAIDL) {
		Intent intent = new Intent();
		Bundle bundle = new Bundle();
		try {
			...
			String packageName = settingPreference.getSettingParam(
					Define.THIRD_PACKAGE, context.getPackageName());
			boolean FairCopy = settingPreference.getSettingParam(
					Define.FAIR_COPY, true);
			String userName = settingPreference.getSettingParam(
					Define.USER_NAME, "");
			...
			bundle.putString(Define.USER_NAME, userName);
			bundle.putString(Define.THIRD_PACKAGE, packageName);
			bundle.putBoolean(Define.BACK_KEY_DOWN, settingPreference
					.getSettingParam(Define.BACK_KEY_DOWN, true));
			bundle.putBoolean(Define.HOME_KEY_DOWN, settingPreference
					.getSettingParam(Define.HOME_KEY_DOWN, true));
	...
	public static boolean isETFile(String filePath) {
  		filePath = filePath.toLowerCase();
  		return filePath.endsWith(".et")
	...
	public static boolean IsZipFile(File file) {
		String end = file
				...
				.substring(file.getName().lastIndexOf(".") + 1,
						file.getName().length()).toLowerCase();
		if (end.equals("zip") || end.equals("ZIP") || end.equals("rar")
				|| end.equals("RAR"))
			return true;
